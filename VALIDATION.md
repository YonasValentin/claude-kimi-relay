...
A security and correctness audit was applied on 2026-07-23 (see the CHANGELOG hardening entries). On the merged `main`, `npm run verify` passes with the suite in `tests/` green, the committed `plugin/dist` matches a fresh build, and a live `review -> delegate -> apply` round-trip against `kimi acp` 0.29.0 completed correctly with the original working tree untouched in every mode. The live runs exercised the happy path, workspace isolation, and the patch boundary on macOS; the Windows process-group termination path and adversarial deny paths are covered by unit tests rather than live runs.
